...
export async function sendSignInLinkEmail({
  ...
  expiresAt,
  userType = "existing",
  language = "en", // ✅ CORRECT - with default value
  ...
  // Professional HTML email template with maroon and cream color scheme
  const htmlContent = `
<!DOCTYPE html>
<html lang="${language}">
<head>
...
${t.footerSentTo} ${to} ${t.footerReason}
  ...
  const mailOptions = {
    ...
    to: to,
    subject: t.subjectLine, // ✅ Now using translated subject
    text: textContent,
    html: htmlContent,
    headers: {
      "x-priority": "3",
      ...
      importance: "Normal",
      "Auto-Submitted": "auto-generated", // Prevents auto-responder loops
      "X-Auto-Response-Suppress": "OOF, AutoReply", // Suppresses out-of-office replies
  ...
  try {
    ...
    console.error("❌ Sign-in email sending failed:", error);
    throw new Error(
